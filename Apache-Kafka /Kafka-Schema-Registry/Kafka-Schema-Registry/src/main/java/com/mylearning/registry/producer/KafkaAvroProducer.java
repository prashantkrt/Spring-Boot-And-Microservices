package com.mylearning.registry.producer;
...
public class KafkaAvroProducer {
    ...
    public void send(Employee employee){
       // CompletableFuture<SendResult<String, Employee>> future = template.send(topicName, 1,UUID.randomUUID().toString(),employee);
        CompletableFuture<SendResult<String, Employee>> future = template.send(topicName, 1,UUID.randomUUID().toString(),employee);
